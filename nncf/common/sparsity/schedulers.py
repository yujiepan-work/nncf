...
class ModifiedPolynomialThresholdScheduler(BaseCompressionScheduler):
    ...
    def __init__(self, controller: SparsityController, params: dict):
        ...
        super().__init__()
        self._controller = controller
        self.warmup_start_scale = params.get('warmup_start_scale', 0.0)
    ...
    @property
    def current_importance_lambda(self):
        if self.in_warmup and (not self.do_regularization_factor_warmup):
            return self.importance_regularization_factor * self.warmup_end_scale
        else:
            return self.importance_regularization_factor * self.current_scale
    ...
    def schedule_threshold(self):
        if self.current_step < self.warmup_start_epoch * self._steps_per_epoch:
            ...
            self.in_warmup = False
            self.current_scale  = self.warmup_end_scale
            self._disable_importance_grad()
            ...
            self.in_warmup = True
            self.current_scale  = self._calculate_threshold_level()
